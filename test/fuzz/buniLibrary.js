...
const MockBuniCornPool = artifacts.require('MockBuniCornPool');
const BuniCornPool = artifacts.require('BuniCornPool');

const BuniCornLibrary = artifacts.require('MockBuniCornLibrary');
...
contract('BuniCornLibrary', function (accounts) {
  beforeEach('set up', async function () {
    library = await BuniCornLibrary.new();
  ...
  it('sortToken', async () => {
    ...
    const pool = await BuniCornPool.at(poolAddresses[0]);
    expect(await pool.token0(), token0.address);

    await expectRevert(library.sortTokens(token0.address, token0.address), 'BUNILIB: IDENTICAL_ADDRESSES');
    await expectRevert(library.sortTokens(token0.address, constants.ZERO_ADDRESS), 'BUNILIB: ZERO_ADDRESS');
  });
  ...
  for (let index = 0; index < 10; index++) {
    it(`fuzz test getAmountOut index=${index}`, async function () {
      ...
      await expectRevert(pool.swap(0, amountOut.add(new BN(1)), accounts[0], '0x'), 'BUNI: K');
  ...
  for (let index = 0; index < 10; index++) {
    it(`fuzz test getAmountIn index=${index}`, async function () {
      ...
      await expectRevert(pool.swap(0, amountOut, accounts[0], '0x'), 'BUNI: K');
...
async function setupPool (admin, library, isAmpPool) {
  let factory = await BuniCornFactory.new(admin);
  ...
  const pool = await MockBuniCornPool.new(factory.address, token0.address, token1.address, isAmpPool);
