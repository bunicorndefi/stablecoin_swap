...
contract('BuniCornPool', function (accounts) {
  ...
  describe('mint', async () => {
    it('unamplified pool', async () => {
      ...
      await expectRevert(pool.mint(trader, {from: app}), 'BUNI: INSUFFICIENT_LIQUIDITY_MINTED');
  ...
  describe('fee', async () => {
    it('feeTo:on unamplified pool', async () => {
      ...
      let fee = await buniHelper.getFee(totalSuppy, k, kLast, governmentFeeBps);
      ...
      amountOut = await buniHelper.getAmountOut(swapAmount, token1, pool);
      await token1.transfer(pool.address, swapAmount);
      await pool.swap(amountOut, 0, trader, '0x');
    ...
    it('feeTo:on amp pool', async () => {
      ...
      let fee = buniHelper.getFee(totalSuppy, k, kLast, governmentFeeBps);
  ...
  describe('sync', async () => {
    it('case 1: donation from 1 side', async () => {
      ...
      let tradeInfo = await pool.getTradeInfo();
      let priceRange = buniHelper.getPriceRange(tradeInfo);
      ...
      tradeInfo = await pool.getTradeInfo();
      priceRange = buniHelper.getPriceRange(tradeInfo);
      console.log(`minRate=${priceRange[0].toString()} maxRate=${priceRange[1].toString()}`);
    });
    ...
    it('case 2: donation from 2 side -> reserve data should scale up', async () => {
      ...
      tradeInfo = await pool.getTradeInfo();
      let priceRange2 = buniHelper.getPriceRange(tradeInfo);
